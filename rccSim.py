...
rho,esd, gtype, eprob = 0.10,0.05,"random",0.5
...
#Group level matrices        
      ...
      for g in range(0,G):
         ...
         ##uses rws and cols for lower triangular matrix mentioned before
          ...
          # Adding in numShare shared edges
            ...
            Omega0s_previous = Omega0s
            #print(Omega0s_previous)
            
            if g > 0: #python starts indexing at 0 compared to 1 in R
               for e in range(len(eshare)):
                  Omega0s[eshare[e, 0], eshare[e, 1]] = Omega0s_previous[eshare[e, 0], eshare[e, 1]]
            #print(Omega0s)
            ...
            Omega0s = Omega0s_s
            value,vector = eig(Omega0s)
            f = min(value) #to update the while loop 
      #print(Omega0s_list)
            
    vl,vc = eig(Omegaks)                
    min_eigenvalue = min(val)
    ...
    #Also uses lower triangular(d=F) rows and cols mentioned above
          ...
          if x > 0:
              ...
              for s in range(len(swaps)):
               ...
               if gks[swaps[s, 0], swaps[s, 1]] == 1:
                   Omegaks[swaps[s, 0], swaps[s, 1]] = np.multiply(np.random.uniform(low = 0.5, high = 1, size = 1), np.random.choice((-1,1), 1))
               else:
     ...
     # Making matrix symmetric and positive definite
          ...
          vu,vr = eig(Omegaks_symm)
          min_eigenvalue = min(vu) #to update the while loop
    #print(len(gks_list))
